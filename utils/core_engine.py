...
def _ensure_cpa_tunnel() -> bool:
    ...
    try:
        ...
        if resp.status_code == 200:
            return True
        print(
            f"[{ts()}] [WARNING] Tunnel manager ensure failed: HTTP {resp.status_code} {resp.text}"
        )
...
def test_cliproxy_auth_file(
    item: dict, api_url: str, api_token: str
) -> Tuple[bool, str]:
    ...
    try:
        ...
        reason = _extract_cliproxy_failure_reason(
            data, cfg.MIN_REMAINING_WEEKLY_PERCENT
        )
    ...
    try:
        ...
        if resp.status_code != 200:
            if resp.status_code == 401:
                return False, "凭证无效 (HTTP 401)"
            if resp.status_code == 403:
                return False, "请求被拒绝 (HTTP 403)"
            return False, f"HTTP {resp.status_code}"
...
def process_account_worker(i: int, total: int, item: dict, args: Any) -> bool:
    ...
    if is_ok:
        if is_disabled:
            ...
            if not can_reenable:
                print(
                    f"[{ts()}] [INFO] 测活: {mask_email(name)} 额度尚未恢复（{reason}），继续保持禁用状态。"
                )
                return False
            print(
                f"[{ts()}] [INFO] 测活: {mask_email(name)} 额度已恢复且有效，准备启用..."
            )
            ok = set_cpa_auth_file_status(
                cfg.CPA_API_URL, cfg.CPA_API_TOKEN, name, disabled=False
            )
            print(
    ...
    if "周限额" in msg or "usage_limit_reached" in msg:
        if cfg.REMOVE_ON_LIMIT_REACHED:
            ...
            ok = set_cpa_auth_file_status(
                cfg.CPA_API_URL, cfg.CPA_API_TOKEN, name, disabled=True
            )
            print(
    ...
    if item.get("runtime_only") or item.get("source") == "memory":
        ...
        try:
            ...
            if content_resp.status_code != 200:
                print(
                    f"[{ts()}] [ERROR] 获取 {mask_email(name)} 完整内容失败 "
    ...
    if refresh_token_val:
        ...
        if ok:
            print(
                f"[{ts()}] [INFO] {mask_email(name)} Token 刷新成功，正在同步至CPA..."
            )
            ...
            if up_ok:
                time.sleep(3)
                is_ok2, msg2 = test_cliproxy_auth_file(
                    item, cfg.CPA_API_URL, cfg.CPA_API_TOKEN
                )
                if is_ok2:
                    refresh_success = True
                    print(
                        f"[{ts()}] [SUCCESS] 测活: {mask_email(name)} 刷新后复活成功！"
                    )
                else:
                    print(
                        f"[{ts()}] [WARNING] {mask_email(name)} 刷新后二次测活依然失败({msg2})"
                    )
            else:
                print(f"[{ts()}] [ERROR] 刷新后覆盖CPA失败: {up_msg}")
        else:
            print(
                f"[{ts()}] [WARNING] {mask_email(name)} Token 复活请求被拒绝: "
...
def _handle_dead_account(name: str, is_disabled: bool) -> None:
    ...
    if cfg.REMOVE_DEAD_ACCOUNTS:
        ...
        print(
            f"[{ts()}] [INFO] 凭证 {mask_email(name)} 死亡，根据配置保留，正在禁用..."
        )
        if set_cpa_auth_file_status(
            cfg.CPA_API_URL, cfg.CPA_API_TOKEN, name, disabled=True
        ):
            print(f"[{ts()}] [SUCCESS] 死亡凭证 {mask_email(name)} 已成功禁用。")
    else:
        print(
            f"[{ts()}] [WARNING] 凭证 {mask_email(name)} 已死亡，当前已是禁用状态，根据配置保留不删除。"
        )


def handle_registration_result(
    result: Any, cpa_upload: bool = False, run_ctx: dict = None
) -> str:
    ...
    if not token_json_str or token_json_str == "retry_403":
        ...
        if cfg.ENABLE_SUB_DOMAINS:
            ...
            print(
                f"[{ts()}] [系统] 域名 {mask_email(cur_dom or '')} 注册失败，下一轮重新生成。"
            )
        ...
        if should_sync:
            if db_manager.save_account_to_db(account_email, password, token_json_str):
                print(
                    f"[{ts()}] [SUCCESS] [{mode_label}] 账号密码与 Token 已安全存入: {mask_email(account_email)}"
                )

        # CPA 云端上传
        if cpa_upload:
            success, up_msg = upload_to_cpa_integrated(
                token_data, cfg.CPA_API_URL, cfg.CPA_API_TOKEN
            )
            if success:
                print(
                    f"[{ts()}] [SUCCESS] 补货凭证 {mask_email(account_email)} 云端上传成功！"
                )
            else:
                print(f"[{ts()}] [ERROR] 云端上传失败: {up_msg}")

        if (
            getattr(cfg, "LOCAL_MS_POOL_FISSION", False)
            and cfg.EMAIL_API_MODE == "local_microsoft"
        ):
            db_manager.update_pool_fission_result(
                master_email, is_blocked=False, is_raw=is_raw
            )
        elif (
            not getattr(cfg, "LOCAL_MS_ENABLE_FISSION", False)
            and cfg.EMAIL_API_MODE == "local_microsoft"
        ):
        ...
        orig_masked_password = (
            f"{safe_pwd[:2]}****{safe_pwd[-2:]}" if len(safe_pwd) > 4 else "****"
        )

        final_email = (
            orig_masked_email
            if getattr(cfg, "TG_BOT", {}).get("mask_email", False)
            else account_email
        )
        final_password = (
            orig_masked_password
            if getattr(cfg, "TG_BOT", {}).get("mask_password", False)
            else safe_pwd
        )

        template_str = getattr(cfg, "TG_BOT", {}).get(
            "template_success", "成功: {email} / {password} 时间: {time}"
        )
        beijing_tz = timezone(timedelta(hours=8))
        current_time = datetime.now(beijing_tz).strftime("%Y-%m-%d %H:%M:%S")
        try:
            success_text = template_str.format(
                email=final_email, password=final_password, time=current_time
            )
        except Exception:
...
def _handle_sub2api_dead_account(item: dict, client: Any, is_disabled: bool) -> None:
    ...
    if cfg.SUB2API_REMOVE_DEAD_ACCOUNTS:
        ...
        print(
            f"[{ts()}] [ERROR] 凭证 {mask_email(name)} 死亡，根据配置保留，正在禁用..."
        )
        if hasattr(client, "set_account_status") and account_id:
            client.set_account_status(account_id, disabled=True)
    else:
        print(
            f"[{ts()}] [ERROR] 凭证 {mask_email(name)} 已死亡，当前已是禁用状态，根据配置保留不删除。"
        )


def process_sub2api_worker(
    i: int, total: int, item: dict, client: Any, args: Any
) -> bool:
    ...
    if result == "quota":
        if cfg.SUB2API_REMOVE_ON_LIMIT_REACHED:
            print(
                f"[{ts()}] [WARNING] Sub2API测活: {mask_email(name)} 额度耗尽，执行物理删除..."
            )
            if account_id:
                client.delete_account(account_id)
            return False
        print(
            f"[{ts()}] [WARNING] Sub2API测活: {mask_email(name)} 额度限流，暂不计入有效库存，Sub2API 自动管理"
        )
        return False
        ...
        try:
            if cfg._clash_enable and not cfg._clash_pool_mode:
                ...
                if not smart_switch_node(args.proxy):
                    print(
                        f"[{ts()}] [WARNING] 全局节点切换失败，将使用当前 IP 继续尝试..."
                    )
            ...
            if cfg.ENABLE_MULTI_THREAD_REG:
                ...
                def _worker():
                    ...
                    if cfg.is_raw_proxy_pool_enabled():
                        borrowed_generation, p = cfg.unpack_proxy_queue_item(
                            cfg.PROXY_QUEUE.get()
                        )
                        try:
                            ...
                            if cfg.should_return_pooled_proxy(borrowed_generation):
                                cfg.PROXY_QUEUE.put(
                                    cfg.make_proxy_queue_item(p, borrowed_generation)
                                )
                    ...
                    if cfg._clash_enable and cfg._clash_pool_mode:
                        ...
                        try:
                            return run_and_refresh(
                                proxy_url, args, False, skip_switch=False
                            )
                        finally:
    ...
    if executor is not None:
        futures = [
            loop.run_in_executor(
                executor, process_account_worker, i, total_files, item, args
            )
        ...
        with ThreadPoolExecutor(max_workers=cfg.CPA_THREADS) as _ex:
            futures = [
                loop.run_in_executor(
                    _ex, process_account_worker, i, total_files, item, args
                )
                for i, item in enumerate(codex_files, 1)
            ]
            results = await asyncio.gather(*futures)
...
async def perform_sub2api_check(args, async_stop_event, loop, client, executor=None):
    ...
    if executor is not None:
        futures = [
            loop.run_in_executor(
                executor, process_sub2api_worker, i, total_files, item, client, args
            )
        ...
        with ThreadPoolExecutor(max_workers=cfg.SUB2API_THREADS) as _ex:
            futures = [
                loop.run_in_executor(
                    _ex, process_sub2api_worker, i, total_files, item, client, args
                )
                for i, item in enumerate(account_list, 1)
            ]
            results = await asyncio.gather(*futures)
    ...
    valid_count = sum(1 for r in results if r)
    print(
        f"[{ts()}] [INFO] Sub2API 测活结束，当前有效数: {valid_count} / {total_files}"
    )
    return valid_count, total_files
...
async def manual_check_main_loop(args, async_stop_event: asyncio.Event, executor=None):
    ...
    if cfg.ENABLE_CPA_MODE:
        check_task = asyncio.create_task(
            perform_cpa_check(args, async_stop_event, loop, executor=executor)
        )
    elif cfg.ENABLE_SUB2API_MODE:
        client = Sub2APIClient(api_url=cfg.SUB2API_URL, api_key=cfg.SUB2API_KEY)
        check_task = asyncio.create_task(
            perform_sub2api_check(
                args, async_stop_event, loop, client, executor=executor
            )
        )
    else:
    ...
    if check_task:
        stop_task = asyncio.create_task(async_stop_event.wait())
            ...
            [check_task, stop_task], return_when=asyncio.FIRST_COMPLETED
        )
        ...
        try:
            if cfg.CPA_AUTO_CHECK:
                valid_count, total_files = await perform_cpa_check(
                    args, async_stop_event, loop, executor=executor
                )
            else:
                print(
                ...
                print(
                    f"[{ts()}] [INFO] 当前云端总数: {total_files} (未开启自动巡检，默认全部视为有效)"
                )
            ...
            if valid_count < cfg.MIN_ACCOUNTS_THRESHOLD:
                ...
                print(
                    f"[{ts()}] [INFO] 库存不足 ({valid_count} < {cfg.MIN_ACCOUNTS_THRESHOLD})，启动补货..."
                )
                ...
                def _cpa_worker():
                    ...
                    if cfg.is_raw_proxy_pool_enabled():
                        ...
                        try:
                            return run_and_refresh(
                                p, args, cpa_upload=True, skip_switch=True
                            )
                        finally:
                            if cfg.should_return_pooled_proxy(borrowed_generation):
                                cfg.PROXY_QUEUE.put(
                    ...
                    if cfg._clash_enable and cfg._clash_pool_mode:
                        ...
                        try:
                            return run_and_refresh(
                                proxy_url, args, cpa_upload=True, skip_switch=False
                            )
                        finally:
                            cfg.PROXY_QUEUE.put(p)
                            cfg.PROXY_QUEUE.task_done()
                    return run_and_refresh(
                        args.proxy, args, cpa_upload=True, skip_switch=True
                    )

                while (
                    success_in_this_cycle < need_to_reg
                    and not async_stop_event.is_set()
                    and not cfg.POOL_EXHAUSTED
                ):
                    ...
                    if cfg._clash_enable and not cfg._clash_pool_mode:
                        ...
                        if not smart_switch_node(args.proxy):
                            print(
                                f"[{ts()}] [WARNING] [CPA补货] 全局节点切换失败，使用当前 IP 继续..."
                            )

                    if cfg.ENABLE_MULTI_THREAD_REG:
                        print(
                            f"[{ts()}] [INFO] 多线程补货: {success_in_this_cycle}/{need_to_reg} "
                            f"({batch_size} 线程)"
                        )
                        if executor is not None:
                            reg_futures = [
                        ...
                        for status in reg_results:
                            if status == "success":
                                ...
                                print(
                                    f"[{ts()}] [WARNING] 遇到 403 频率限制，给服务器 15 秒冷却时间..."
                                )
                                await asyncio.sleep(15)
                    else:
                        print(
                            f"[{ts()}] [INFO] 单线程补货: {success_in_this_cycle}/{need_to_reg}"
                        )
                        if cfg.is_raw_proxy_pool_enabled():
                            borrowed_generation, p = cfg.unpack_proxy_queue_item(
                                cfg.PROXY_QUEUE.get()
                            ...
                            try:
                                status = await loop.run_in_executor(
                                    None, run_and_refresh, p, args, True, True
                                )
                            finally:
                                if cfg.should_return_pooled_proxy(borrowed_generation):
                                    cfg.PROXY_QUEUE.put(
                                        cfg.make_proxy_queue_item(
                                            p, borrowed_generation
                                        )
                                    )
                                    cfg.PROXY_QUEUE.task_done()
                        elif cfg._clash_enable and cfg._clash_pool_mode:
                            ...
                            try:
                                status = await loop.run_in_executor(
                                    None, run_and_refresh, proxy_url, args, True, False
                                )
                            finally:
                                cfg.PROXY_QUEUE.put(p)
                                cfg.PROXY_QUEUE.task_done()
                    ...
                    if cfg.EMAIL_API_MODE in ["local_microsoft", "gmail_fission"]:
                        global_postman_fleet.clear_fleet()
                print(
                    f"[{ts()}] [SUCCESS] 本轮补货完成！累计入库: {success_in_this_cycle} 个。"
                )
            else:
            ...
            print(
                f"[{ts()}] [INFO] 维护周期结束，{cfg.CHECK_INTERVAL_MINUTES} 分钟后进行下一次巡检..."
            )
            try:
    ...
    while not async_stop_event.is_set() and not cfg.POOL_EXHAUSTED:
        try:
            if cfg.SUB2API_AUTO_CHECK:
                ...
                valid_count = sum(1 for r in results if r)
                print(
                    f"[{ts()}] [INFO] 巡检结束，当前 Sub2API 仓库有效数: {valid_count}"
                )
            else:
                print(
                    f"[{ts()}] [INFO] Sub2API 自动测活已关闭，直接读取云端列表进行补发判断..."
                )
                success, account_list = client.get_all_accounts()
                if not success:
                    print(f"[{ts()}] [ERROR] 获取 Sub2API 全量库存失败: {account_list}")
            ...
            if valid_count < cfg.SUB2API_MIN_THRESHOLD:
                ...
                success_in_this_cycle = 0
                print(
                    f"[{ts()}] [INFO] 库存不足 ({valid_count} < {cfg.SUB2API_MIN_THRESHOLD})，启动补货..."
                )
                await asyncio.sleep(1)
                ...
                def _sub2api_run_wrapper(p, skip_switch):
                    ...
                    if not skip_switch:
                        if not smart_switch_node(p):
                            print(
                                f"[{ts()}] [WARNING] [Sub2API补货] 全局节点切换失败..."
                            )
                    run_ctx = {}
                    result = run(p, run_ctx=run_ctx)
                    status = handle_registration_result(
                        result, cpa_upload=False, run_ctx=run_ctx
                    )
                ...
                while (
                    ...
                    if cfg._clash_enable and not cfg._clash_pool_mode:
                        ...
                        if not smart_switch_node(args.proxy):
                            print(
                                f"[{ts()}] [WARNING] [Sub2API补货] 全局节点切换失败，使用当前 IP 继续..."
                            )
                    ...
                    if cfg.ENABLE_MULTI_THREAD_REG:
                        ...
                        for status in reg_results:
                            if status == "success":
                                ...
                                try:
                                    await asyncio.wait_for(
                                        async_stop_event.wait(), timeout=15
                                    )
                                except asyncio.TimeoutError:
                                    pass
                        ...
                        if cfg.is_raw_proxy_pool_enabled():
                            ...
                            try:
                                status = await loop.run_in_executor(
                                    None, _sub2api_run_wrapper, p, True
                                )
                            finally:
                                if cfg.should_return_pooled_proxy(borrowed_generation):
                                    cfg.PROXY_QUEUE.put(
                            ...
                            try:
                                status = await loop.run_in_executor(
                                    None, _sub2api_run_wrapper, proxy_url, False
                                )
                            finally:
                                cfg.PROXY_QUEUE.put(p)
                                cfg.PROXY_QUEUE.task_done()
                        ...
                        if status == "success":
                            ...
                            try:
                                await asyncio.wait_for(
                                    async_stop_event.wait(), timeout=10
                                )
                            except asyncio.TimeoutError:
                                pass
                    ...
                    if cfg.EMAIL_API_MODE in ["local_microsoft", "gmail_fission"]:
                        global_postman_fleet.clear_fleet()
                print(
                    f"[{ts()}] [SUCCESS] 本轮补货完成！累计入库 Sub2API: {success_in_this_cycle} 个。"
                )
            else:
                print(f"[{ts()}] [INFO] 仓库存量充足，无需补发。")
            ...
            if async_stop_event.is_set() or getattr(cfg, "GLOBAL_STOP", False):
                ...
                break
            print(
                f"[{ts()}] [INFO] 维护周期结束，{cfg.SUB2API_CHECK_INTERVAL} 分钟后进行下一次巡检..."
            )
            try:
                await asyncio.wait_for(
                    async_stop_event.wait(),
...
class RegEngine:
    ...
    def _ensure_executor(self, max_workers=None):
        if self._executor is None:
            workers = max_workers or max(
                cfg.REG_THREADS,
                getattr(cfg, "CPA_THREADS", 4),
                getattr(cfg, "SUB2API_THREADS", 4),
            )
    ...
    def _run_sub2api_in_thread(self, args):
        ...
        try:
            ...
            self.loop.run_until_complete(
                sub2api_main_loop(args, self.async_stop_event, executor=self._executor)
            )
        finally:
            self._finalize_thread_run()
    ...
    def _run_check_in_thread(self, args):
        ...
        try:
            self.async_stop_event = asyncio.Event()
            self.loop.run_until_complete(
                manual_check_main_loop(
                    args, self.async_stop_event, executor=self._executor
                )
            )
        finally:
            self._finalize_thread_run()
